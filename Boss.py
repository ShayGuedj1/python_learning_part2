...
class BadBoss(Boss):
    ...
    def hoard_praise(self):
        ...
        print("The employees feel cheated and start plotting {}'s demise while he stares at his own reflection.".format(
            self.name))

    def yell(self):
        # A bad boss yells! (Ain't nobody got time for that!)
        print("Everyone stares while {} yells. Someone shouts, 'Won't somebody PLEASE think of the children!'".format(
            self.name))
        print(
            "{} storms off, everyone comforts the victim and one person offers to arrange an 'accident' for {}.".format(
                self.name, self.name))
